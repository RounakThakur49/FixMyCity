...
- **Backend** (`backend/`): Express + MongoDB (Mongoose) + JWT API on port 5000. **No ML dependencies** — image validation is delegated over HTTP to the ML service (`POST ${ML_SERVICE_URL}/api/infer`), fail-open if it's unreachable.
- **ML service** (`ml-service/`): standalone Express inference service on port 7860 holding the entire multi-stage ML pipeline (custom EfficientNetV2S civic classifier + NSFW content moderation + CLIP open-set classifier). Called server-to-server by the backend; guarded by an `X-ML-KEY` shared secret. Deploy target: Hugging Face Spaces (Docker).
