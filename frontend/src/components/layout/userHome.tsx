...
import { fetchUserHomeRequest } from '../../actions/citizen.slice';
...
export default function UserHome({ onRegisterClick }: UserHomeProps) {
    const dispatch = useAppDispatch();
    const { user } = useSelector((state: RootState) => state.auth);
    const { loading, error, registeredComplaints, statusMap } = useSelector((state: RootState) => state.citizen);
    ...
    const sortedComplaints = React.useMemo(() => {
        return [...registeredComplaints].reverse();
    ...
    const counts = React.useMemo(() => {
        ...
        registeredComplaints.forEach((complaint: any) => {
            const latest = statusMap && statusMap[complaint.id || complaint._id];
            const rawStatus = (latest ? latest.workstatus : (complaint.status || 'pending')).toLowerCase();
        ...
        return { pending, progress, completed, total: registeredComplaints.length };
    }, [registeredComplaints, statusMap]);

    const latestUpdatedComplaint = React.useMemo(() => {
        if (sortedComplaints.length === 0) return null;
        
        let latestComp = sortedComplaints[0];
        let newestDate = new Date(0);
        
        sortedComplaints.forEach((comp: any) => {
            const updateObj = statusMap && statusMap[comp.id || comp._id];
            if (updateObj) {
                const updateDateStr = updateObj.createdAt || updateObj.date || comp.createdAt || comp.date;
                const updateDate = updateDateStr ? new Date(updateDateStr) : new Date(0);
                if (updateDate > newestDate) {
    ...
    // Auto-sync every 15s — silently refreshes data without showing a spinner.
    // Pauses when complaint details dialog is open.
    useEffect(() => {
        if (!userId) return;
    ...
    }, [userId, role, dispatch, selectedComplaint]);
    ...
    return (
        <Box sx={{
            ...
            {/* Header section with register complaint action */}
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
                <Box>
                    ...
                    <Typography variant="subtitle1" color="#64748b">
                        Welcome back, {loginUserObj?.user?.name || 'Citizen'}! Track and submit your complaints.
                    </Typography>
                </Box>
                <Button
                    variant="contained"
            ...
            {!loading && !error && registeredComplaints.length === 0 && (
                <Box sx={{ py: 8, textAlign: 'center', backgroundColor: '#f8fafc', borderRadius: 4, border: '1px dashed #cbd5e1' }}>
                    ...
                    <Typography color="#94a3b8" variant="body2" sx={{ mt: 1 }}>
                        Click "Register Complaint" above to report an issue in your city.
                    </Typography>
                </Box>
            )}
                ...
                >
                    ...
                    <Box sx={{ mb: 2 }}>
                        <TextField
                            fullWidth
                            size="small"
                            placeholder="Search by complaint ID, title, description, or category..."
                            value={searchQuery}
                        ...
                        {(statusFilter !== 'all' || searchQuery.trim()) && (
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
                                <Typography variant="caption" sx={{ color: '#64748b' }}>
                                    Showing {filteredComplaints.length} of {registeredComplaints.length} complaints
                                </Typography>
                                <Chip
                                    label="Clear filters"
                                    size="small"
                    ...
                    <Grid container spacing={4} sx={{ flex: 1, minHeight: 0 }}>
                        {/* Left Column - Latest Status Update Card */}
                        <Grid size={{ xs: 12, md: 5, lg: 5 }} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                            <Box sx={{
                                ...
                                <Box sx={{ 
                                    ...
                                    {latestUpdatedComplaint && (
                                        <Chip 
                                            label={(latestUpdatedComplaint.status || 'Pending').toLowerCase().includes('progress') ? 'Work on Progress' : (latestUpdatedComplaint.status || 'Pending').toLowerCase().includes('complete') ? 'Completed' : 'Pending'}
                                            size="small"
                                            sx={{
                                                background: (latestUpdatedComplaint.status || 'Pending').toLowerCase().includes('complete') 
                                                    ? 'linear-gradient(135deg, #22c55e 0%, #15803d 100%)' 
                                                    : (latestUpdatedComplaint.status || 'Pending').toLowerCase().includes('progress') 
                                                        ? 'linear-gradient(135deg, #3b82f6 0%, #eab308 100%)' 
                                                        : 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)',
                                                color: '#ffffff',
                                                fontWeight: 700,
                                ...
                                {/* Panel Body - Scrollable content in case text is long */}
                                    ...
                                    {latestUpdatedComplaint ? (
                                        <>
                                            ...
                                            <Box>
                                                ...
                                                <Typography variant="h6" sx={{ fontWeight: 800, color: '#0f172a', mt: 0.5, mb: 1 }}>
                                                    {latestUpdatedComplaint.title}
                                                </Typography>
                                                <Chip 
                                                    label={latestUpdatedComplaint.issueType || latestUpdatedComplaint.issuetype || 'General'} 
                                                    size="small" 
                                                    variant="outlined" 
                                                    sx={{ borderColor: '#cbd5e1', color: '#475569', fontWeight: 600, fontSize: '0.7rem' }}
                                                />
                                                <Typography variant="body2" sx={{ color: '#475569', mt: 2, lineBreak: 'anywhere', backgroundColor: '#f8fafc', p: 2, borderRadius: 2, border: '1px solid #f1f5f9' }}>
                                                    {latestUpdatedComplaint.description}
                                                </Typography>
                                            </Box>

                                            <Box sx={{ width: '100%', height: '1px', backgroundColor: '#e2e8f0' }} />

                                            {/* Section 2: Work Status Update Info */}
                                            <Box>
                                                <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                                    Work Update Details
                                                </Typography>
                                                
                                                {statusMap && statusMap[latestUpdatedComplaint.id || latestUpdatedComplaint._id] ? (
                                                    <Box sx={{ mt: 1.5 }}>
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
                                                            ...
                                                            <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#0f172a' }}>
                                                                {statusMap[latestUpdatedComplaint.id || latestUpdatedComplaint._id].note || 'No description provided'}
                                                            </Typography>
                                                        </Box>
                                                        
                                                        {statusMap[latestUpdatedComplaint.id || latestUpdatedComplaint._id].createdAt && (
                                                            <Typography variant="caption" sx={{ color: '#94a3b8', display: 'block', mb: 2, pl: 3.5 }}>
                                                                Updated on: {new Date(statusMap[latestUpdatedComplaint.id || latestUpdatedComplaint._id].createdAt).toLocaleString()}
                                                            </Typography>
                                                        )}

                                                        {(() => {
                                                            const src = statusMap[latestUpdatedComplaint.id || latestUpdatedComplaint._id].photoUrl || '';
                                                            if (!src) return null;
                                                            const displaySrc = (src.startsWith('data:') || src.startsWith('http') || src.startsWith('blob:'))
                                                                ? src
                                                                : `/photo/${src}`;
                                                            return (
                                                                <Box sx={{ mt: 2, borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0', pl: 3.5 }}>
                                                                    <CardMedia
                                                                        component="img"
                                                                        sx={{ maxHeight: 200, width: '100%', objectFit: 'cover', borderRadius: '8px' }}
                                                                        image={displaySrc}
                                                    ...
                                                    <Typography variant="body2" sx={{ color: '#94a3b8', mt: 1.5, fontStyle: 'italic' }}>
                                                        No work updates submitted yet. The complaint is currently pending action from the administration.
                                                    </Typography>
                                                )}
                                            </Box>
                                        </>
                                        ...
                                        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', py: 8 }}>
                                            <Typography variant="body2" color="#94a3b8" sx={{ fontStyle: 'italic' }}>
